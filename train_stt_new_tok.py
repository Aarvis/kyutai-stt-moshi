...
def _train(args: TrainArgs, exit_stack: ExitStack):
    ...
    #Re-init embedding + head (same vocab size, entirely fresh)
    # 1) New vocab size & pad id from your SPM
    new_vocab = spm.vocab_size()
    new_pad_id = spm.pad_id() if spm.pad_id() >= 0 else getattr(model, "text_padding_token_id", 0)

    # 2) Find current embedding & head modules
    text_emb = getattr(model, "text_embedding", None) or getattr(model, "text_embeddings", None)
    lm_head  = getattr(model, "lm_head", None) or getattr(model, "text_head", None)
    assert text_emb is not None and lm_head is not None, "Can't find text embedding / lm_head on model."

    d_model = text_emb.embedding_dim
    dev, dtype = text_emb.weight.device, text_emb.weight.dtype

    # 3) Fresh embedding
    new_emb = nn.Embedding(new_vocab, d_model, padding_idx=new_pad_id, device=dev, dtype=dtype)
    nn.init.normal_(new_emb.weight, mean=0.0, std=0.02)
    with torch.no_grad():
        if new_pad_id is not None and new_pad_id >= 0:
            new_emb.weight[new_pad_id].zero_()

    # 4) Fresh LM head (tied or untied)
    new_head = nn.Linear(d_model, new_vocab, bias=False, device=dev, dtype=dtype)
    # If your model ties weights, tie them again:
    try:
        # common pattern
    ...
    # 5) Install
    setattr(model, text_emb.__class__.__name__.lower() if False else "text_embedding", new_emb)
    setattr(model, "lm_head", new_head)

    # 6) Make sure the model's internal ids line up (if your model exposes them)
    if hasattr(model, "text_padding_token_id"):
        model.text_padding_token_id = new_pad_id
